		...
		else if(Msg == NETMSG_SNAP || Msg == NETMSG_SNAPSINGLE || Msg == NETMSG_SNAPEMPTY)
		{
			...
			if(GameTick >= m_CurrentRecvTick[0])
			{
				...
				if(m_SnapshotParts == (unsigned)((1<<NumParts)-1))
				{
					...
					if(m_ReceivedSnapshots[0] > 2)
					{
						int64 Now = m_GameTime[0].Get(time_get());
...
void CClient::Update()
{
	if(State() == IClient::STATE_ONLINE && m_ReceivedSnapshots[0] >= 3)
	{
		...
		{
			int64 CurtickStart = (m_aSnapshots[0][SNAP_CURRENT]->m_Tick)*time_freq()/50;
			int64 PrevtickStart = (m_aSnapshots[0][SNAP_PREV]->m_Tick)*time_freq()/50;
